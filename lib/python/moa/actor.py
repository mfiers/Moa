...
class Actor:
    def __init__(self, wd, 
        ...
        #if captureXXX is not defined determine what to do based on 
        #background
    ...
    def run2(self, cl):
        ...
        l.debug(" - with command line: '%s'" % " ".join(self.cl))
        
        if self.captureOut:
            self.FOUT = open(self.captureOutName, 'w')
        if self.captureErr:
            self.FOUT = open(self.captureErrName, 'w')
